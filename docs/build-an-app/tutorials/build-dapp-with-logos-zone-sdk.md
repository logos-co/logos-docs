...
- The main device operates as a **sequencer**, posting SQL transactions as inscriptions to its [channel](../../get-started/glossary.md#channel).
...
Once the `ZoneSequencer` is set up, posting data to the channel is as simple as passing a byte vector to the sequencer's `publish` function, which returns an [inscription](../../get-started/glossary.md#inscription) ID and the current checkpoint.
