...
In your module's `initLogos()` function, construct `LogosModules` with the provided `LogosAPI*` — as the reference implementation does in [`src/logos_delivery_demo_plugin.cpp`](https://github.com/logos-co/logos-delivery-demo/blob/v0.2.0/src/logos_delivery_demo_plugin.cpp). `LogosModules` is generated at build time by `logos-module-builder`; include it via the umbrella header and keep it as a member on the plugin. The full API reference is in [`src/delivery_module_plugin.h`](https://github.com/logos-co/logos-delivery-module/blob/3258cdb0132e37228aa2519e0c01c0e7429a20dd/src/delivery_module_plugin.h) and the module [README](https://github.com/logos-co/logos-delivery-module/blob/3258cdb0132e37228aa2519e0c01c0e7429a20dd/README.md#module-interface).
...
:::note
The code below follows the hand-written plugin model, where you own the plugin class and its `initLogos()`. The `ui-qml-backend` template of Step 1 uses the newer backend model instead: it generates the plugin class and `initLogos()` around a backend class you write, and you make the same calls through `modules().delivery_module`, arming event subscriptions in `onContextReady()`.
:::
...
1. Register the node event handlers. Every API call is synchronous; delivery outcomes and incoming traffic arrive off-thread via the Qt event loop:

   ```cpp
   m_logos->delivery_module.on("connectionStateChanged", [](const QVariantList& data) {
       // data[0]: QString — connection status
       // data[1]: qint64  — timestamp (ns since epoch)
   });
   ...
   // Register the Reliable Channels API event handlers, if your module uses channels (Step 6):
   m_logos->delivery_module.on("channelMessageReceived", [](const QVariantList& data) {
       ...
       // data[2]: QByteArray  — payload (raw bytes, in causal order)
       // data[3]: qint64      — timestamp (ns since epoch)
   });
...
1. Receive messages through the `messageReceived` event you registered in Step 3. It fires for every message on a subscribed topic, including messages sent by you.
