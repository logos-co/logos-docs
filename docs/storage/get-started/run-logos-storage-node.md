...
1. Install the package into a local modules directory using `lgpm`:
...
1. Load the storage module, initialise it with the testnet configuration, and start it:
...
1. After a second, extract the content ID (CID) from the first `manifests` entry:
...
1. Download the file back from the network with `downloadToUrl`. You need the CID for this step.
...
1. Confirm the downloaded file is present at `<output-path>` and matches the original.
...
1. Wait a bit and destroy the entire storage context:
